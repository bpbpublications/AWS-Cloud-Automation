...
resource "aws_vpc_endpoint" "secretsmanager" {
  ...
  subnet_ids          = [aws_subnet.demo-rdsdb-subnet[0].id]
  security_group_ids  = [aws_security_group.demo-rdsdb-sg.id]
}

data "aws_serverlessapplicationrepository_application" "rotator" {
  #application_id = "arn:aws:serverlessrepo:us-east-2:297356227824:applications/SecretsManagerRDSMySQLRotationSingleUser"
  application_id = "arn:aws:serverlessrepo:us-east-2:758678122039:applications/SecretsManagerRDSMySQLRotationSingleUser"
}
...
resource "aws_serverlessapplicationrepository_cloudformation_stack" "rotate-stack" {
  ...
  parameters = {
    ...
    vpcSubnetIds        = aws_subnet.demo-rdsdb-subnet[0].id
    vpcSecurityGroupIds = aws_security_group.demo-rdsdb-sg.id
  }
}
