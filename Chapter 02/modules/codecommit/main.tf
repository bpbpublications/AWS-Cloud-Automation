resource "aws_codecommit_repository" "source_repository" {
  ...
  description     = "This is the Repository for hosting the terraform code and pipeline configuration files"
  tags            = var.tags
}
resource "aws_codecommit_approval_rule_template" "source_repository_approval" {
  ...
  description = "The production CICD pipline not only have Approval process and Approval rule template for enabling approval process"
