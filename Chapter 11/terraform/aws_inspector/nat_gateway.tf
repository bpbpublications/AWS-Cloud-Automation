# NAT gateway
resource "aws_nat_gateway" "chapter11-inspector-nat-gateway1a" {
  depends_on = [
    aws_subnet.three-tier-public-subnet-1a,
#	aws_subnet.two-tier-public-subnet-1b,
    aws_eip.chapter11-inspector-elastic-ip1,
  ]
  allocation_id = aws_eip.chapter11-inspector-elastic-ip1.id
...
# NAT gateway
resource "aws_nat_gateway" "chapter11-inspector-nat-gateway1b" {
  depends_on = [
#    aws_subnet.two-tier-public-subnet-1a,
        aws_subnet.three-tier-public-subnet-1b,
    aws_eip.chapter11-inspector-elastic-ip2,
  ]
  allocation_id = aws_eip.chapter11-inspector-elastic-ip2.id
