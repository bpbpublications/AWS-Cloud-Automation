 ...
 */
resource "aws_s3_bucket" "cloudtrail-access-log" {
  bucket        = "${var.cloudtrail-access-log}-${var.env}"
...
resource "aws_s3_bucket" "cloudtrail-infra-prod" {
  ...
  logging {
    target_bucket = "${aws_s3_bucket.cloudtrail-access-log.id}"
    target_prefix = "log/"
  }
